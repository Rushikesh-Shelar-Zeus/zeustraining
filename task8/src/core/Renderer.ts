import { Viewport, GridOptions } from '../utils/types.js';
...
export class Renderer {
    ...
    private renderHeaders(
        ...
        const [visibleColStart, visibleColEnd] = this.getVisibleRange(
            colWidths, viewport.scrollX, viewport.width
        );
        const [visibleRowStart, visibleRowEnd] = this.getVisibleRange(
            rowHeights, viewport.scrollY, viewport.height
        );

        // Calculate pixel offset for first visible col/row
        let xOffset = options.headerWidth;
        for (let i = 0; i < visibleColStart; i++) xOffset += colWidths[i];
        let yOffset = options.headerHeight;
        for (let i = 0; i < visibleRowStart; i++) yOffset += rowHeights[i];
